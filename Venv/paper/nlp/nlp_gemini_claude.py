...
def call_gemini(prompt):
    ...
    response = requests.post(
        f"{GEMINI_API_URL}?key={GEMINI_API_KEY}", json=data, headers=headers
    )
...
def detect_text(image):
    ...
    blob = cv2.dnn.blobFromImage(
        gray, 1.0, (320, 320), (123.68, 116.78, 103.94), swapRB=True, crop=False
    )
    net.setInput(blob)
...
def evaluate_student_responses(qp_pdf, subject_pdf, student_responses, model="gemini"):
    ...
    for question, student_answer in student_responses.items():
        ...
        if ideal_answer != "No matching answer found":
            ...
            evaluation_results[question] = {
                "error": "No matching answer found in subject PDF"
            }
